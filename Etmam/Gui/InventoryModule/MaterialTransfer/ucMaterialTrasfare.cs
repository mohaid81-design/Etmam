...
namespace Etmam
{
    public partial class ucMaterialTrasfare : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        public ucMaterialTrasfare()
        {
            ...
            _canManage = PermissionService.HasPermission(PermNames.MaterialTrasfare);
            bool canAddAny = _canManage && PermissionService.CanAccessAnyStore(dcInit, StoreAction.Transfer);
            ...
            // عمود colPrint → طباعة مستند التحويل المُركَّز عليه مباشرة بدون فتح النموذج
            repositoryItemButtonEditPrint.ButtonClick += (s, e) =>
            {
                if (gridView1.GetFocusedRow() is MaterialTransferList row)
                {
                    var handle = ShowOverlay();
                    try { MaterialTransferPrinter.Print(row.Id); }
        ...
        private void LoadLookups()
        {
            ...
            // فلتر المخزن يعرض فقط المخازن المصرَّح للمستخدم فيها بالتحويل (UserStoreAccess.CanTransfer)
            _grantedStoreIds = InventoryStorePermissions.GrantedStoreIds(dc, a => a.CanTransfer);
            ...
            // أعمدة عرض المخزن في الشبكة تبقى بكل المخازن (لعرض اسم المخزن الصحيح لأي سطر بغض النظر عن صلاحية المستخدم الحالي)
        ...
        public void LoadData()
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    filter += $" AND (FromStoreId IN ({ids}) OR ToStoreId IN ({ids}))";
                }

                var data = dc.MaterialTransferList.GetBy(filter, new { PrjId = prjId, StoreId = storeVal }).ToList();
                foreach (var r in data)
                    r.FormattedNum = MaterialTransferPrinter.FormatTransferNumber(r.Num, r.TransferDate);
        ...
        private void OpenAddEdit(int id)
        {
            if (!_canManage)
            {
                XtraMessageBox.Show("ليس لديك صلاحية إدارة مستندات التحويل.", "غير مصرَّح",
        ...
        private void btnOpen_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            if (row == null)
            {
                XtraMessageBox.Show("يرجى تحديد مستند تحويل لفتحه.", "تنبيه", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            using var frm = new frmMaterialTransferAddEdit();
