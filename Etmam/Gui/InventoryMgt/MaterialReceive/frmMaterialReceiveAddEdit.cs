...
namespace Etmam
{
    public class frmMaterialReceiveAddEdit : XtraForm
    {
        private readonly DataContext dc = Data.DataContext.Shared;
        private int _id;
        private MaterialReceiveList? _entity;
        ...
        private RepositoryItemLookUpEdit repoUnits = null!;
        ...
        private void InitializeComponent()
        {
            ...
            this.Text = $"إذن إستلام مواد - {(_id > 0 ? "تعديل" : "جديد")}";

            // Top Panel for Header
            var pnlHeader = new GroupControl { Text = "بيانات المستند الرئيسية", Dock = DockStyle.Top, Height = 200, Margin = new Padding(10) };
            ...
            dtReceivedDate = new DateEdit { EditValue = DateTime.Today, Dock = DockStyle.Fill };
            ...
            txtVoucherNo = new TextEdit { Dock = DockStyle.Fill };
            lkPR = new LookUpEdit { Dock = DockStyle.Fill };
            txtAmount = new TextEdit { ReadOnly = true, Dock = DockStyle.Fill };
            txtDescription = new MemoEdit { Dock = DockStyle.Fill };
            ...
            tableLayout.Controls.Add(new LabelControl { Text = "تاريخ الاستلام:", Anchor = AnchorStyles.Left }, 2, 0);
            tableLayout.Controls.Add(dtReceivedDate, 3, 0);

            tableLayout.Controls.Add(new LabelControl { Text = "المخزن:", Anchor = AnchorStyles.Left }, 0, 1);
            ...
            tableLayout.Controls.Add(new LabelControl { Text = "رقم الفاتورة:", Anchor = AnchorStyles.Left }, 0, 2);
            tableLayout.Controls.Add(txtInvoiceNo, 1, 2);
            tableLayout.Controls.Add(new LabelControl { Text = "رقم سند التسليم:", Anchor = AnchorStyles.Left }, 2, 2);
            tableLayout.Controls.Add(txtVoucherNo, 3, 2);
            ...
            // Grid Layout (Middle)
            var pnlGrid = new GroupControl { Text = "تفاصيل المواد المستلمة", Dock = DockStyle.Fill };
            ...
            var colQty = gridView1.Columns.AddVisible("Qty", "الكمية");
            colQty.Width = 100;
        ...
        private void LoadLookups()
        {
            ...
            repoUnits.DataSource = dc.Units.GetBy("IsDelete = 0");
            repoUnits.ValueMember = "Id";
            repoUnits.DisplayMember = "Description";
            repoUnits.NullText = "";
        }
        ...
        private void LoadRecord()
        {
            if (_id > 0)
            {
                _entity = dc.MaterialReceiveList.Find(_id);
                if (_entity == null)
                {
                    XtraMessageBox.Show("لم يتم العثور على السجل المطلوب.", "خطأ", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    _entity = new MaterialReceiveList();
                }
                
                txtCode.Text = _entity.Code ?? "";
                dtReceivedDate.EditValue = _entity.ReceivedDate;
                lkStore.EditValue = _entity.StoreId;
                lkSupplier.EditValue = _entity.StakeholderId;
                txtInvoiceNo.Text = _entity.InvoiceNo ?? "";
                txtVoucherNo.Text = _entity.ReceiveVoucherNo ?? "";
                txtAmount.Text = (_entity.Amount ?? 0).ToString("N2");

                var list = dc.MaterialReceiveDetails.GetBy("ParentId = @id AND IsDelete = 0", new { id = _id });
                _details = new BindingList<MaterialReceiveDetails>(list);
            }
            else
            {
                _entity = new MaterialReceiveList();
                txtCode.Text = GenerateNextCode();
                dtReceivedDate.EditValue = DateTime.Today;
                lkStore.EditValue = null;
                lkSupplier.EditValue = null;
                txtInvoiceNo.Text = "";
                txtVoucherNo.Text = "";
                txtAmount.Text = "0.00";
                _details = new BindingList<MaterialReceiveDetails>();
            }
        ...
        private string GenerateNextCode()
        {
            try
            {
                var all = dc.MaterialReceiveList.GetBy("IsDelete = 0");
                int maxCode = 0;
                foreach (var rx in all)
                {
                    if (int.TryParse(rx.Code, out int c) && c > maxCode)
                    {
                        maxCode = c;
                    }
                }
        ...
        {
            var row = gridView1.GetRow(e.RowHandle) as MaterialReceiveDetails;
            if (row == null) return;
        ...
        {
            var row = gridView1.GetFocusedRow() as MaterialReceiveDetails;
            if (row == null) return;
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                _entity.ReceivedDate = dtReceivedDate.DateTime;
                _entity.StoreId = lkStore.EditValue as int?;
                _entity.StakeholderId = lkSupplier.EditValue as int?;
                _entity.InvoiceNo = txtInvoiceNo.Text.Trim();
                _entity.ReceiveVoucherNo = txtVoucherNo.Text.Trim();
                ...
                if (_id > 0)
                {
                    ...
                    dc.MaterialReceiveList.Edit(_id, _entity);
                }
                else
                {
                    ...
                    _id = dc.MaterialReceiveList.Add(_entity);
                }

                // Delete removed items
                foreach (int detId in _deletedDetailIds)
                {
                    dc.MaterialReceiveDetails.Delete(detId);
                }
                ...
                foreach (var detail in _details)
                {
                    ...
                    if (detail.Id > 0)
                    {
                        ...
                        dc.MaterialReceiveDetails.Edit(detail.Id, detail);
                    }
                    else
                    {
                        ...
                        dc.MaterialReceiveDetails.Add(detail);
                    }
                }
