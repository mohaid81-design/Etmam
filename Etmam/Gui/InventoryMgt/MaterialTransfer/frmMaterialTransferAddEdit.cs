...
namespace Etmam
{
    public class frmMaterialTransferAddEdit : XtraForm
    {
        private readonly DataContext dc = Data.DataContext.Shared;
        private int _id;
        private MaterialTransferList? _entity;
        ...
        private DateEdit dtTransferDate = null!;
        private LookUpEdit lkFromStore = null!;
        private LookUpEdit lkToStore = null!;
        ...
        private void InitializeComponent()
        {
            ...
            this.Text = $"تحويل بين المخازن - {(_id > 0 ? "تعديل" : "جديد")}";

            // Top Panel for Header
            var pnlHeader = new GroupControl { Text = "بيانات التحويل الرئيسية", Dock = DockStyle.Top, Height = 170, Margin = new Padding(10) };
            ...
            dtTransferDate = new DateEdit { EditValue = DateTime.Today, Dock = DockStyle.Fill };
            lkFromStore = new LookUpEdit { Dock = DockStyle.Fill };
            lkToStore = new LookUpEdit { Dock = DockStyle.Fill };
            ...
            tableLayout.Controls.Add(new LabelControl { Text = "تاريخ التحويل:", Anchor = AnchorStyles.Left }, 2, 0);
            tableLayout.Controls.Add(dtTransferDate, 3, 0);

            tableLayout.Controls.Add(new LabelControl { Text = "من مخزن:", Anchor = AnchorStyles.Left }, 0, 1);
            tableLayout.Controls.Add(lkFromStore, 1, 1);
            tableLayout.Controls.Add(new LabelControl { Text = "إلى مخزن:", Anchor = AnchorStyles.Left }, 2, 1);
            tableLayout.Controls.Add(lkToStore, 3, 1);
            ...
            // Grid Layout (Middle)
            var pnlGrid = new GroupControl { Text = "تفاصيل المواد المحولة", Dock = DockStyle.Fill };
            ...
            var colQty = gridView1.Columns.AddVisible("Qty", "الكمية المحولة");
            colQty.Width = 100;
        ...
        private void LoadLookups()
        {
            ...
            lkFromStore.Properties.DataSource = stores;
            lkFromStore.Properties.ValueMember = "Id";
            lkFromStore.Properties.DisplayMember = "Name";
            lkFromStore.Properties.NullText = "-- اختر مخزن المصدر --";

            lkToStore.Properties.DataSource = stores;
            lkToStore.Properties.ValueMember = "Id";
            lkToStore.Properties.DisplayMember = "Name";
            lkToStore.Properties.NullText = "-- اختر مخزن الوجهة --";
        ...
        private void LoadRecord()
        {
            if (_id > 0)
            {
                _entity = dc.MaterialTransferList.Find(_id);
                if (_entity == null)
                {
                    XtraMessageBox.Show("لم يتم العثور على السجل المطلوب.", "خطأ", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    _entity = new MaterialTransferList();
                }
                
                txtCode.Text = _entity.Code ?? "";
                dtTransferDate.EditValue = _entity.TransferDate;
                lkFromStore.EditValue = _entity.FromStoreId;
                lkToStore.EditValue = _entity.ToStoreId;
                txtNote.Text = _entity.Note ?? "";
                txtAmount.Text = (_entity.Amount ?? 0).ToString("N2");

                var list = dc.MaterialTransferDetails.GetBy("ParentId = @id AND IsDelete = 0", new { id = _id });
                _details = new BindingList<MaterialTransferDetails>(list);
            }
            else
            {
                _entity = new MaterialTransferList();
                txtCode.Text = GenerateNextCode();
                dtTransferDate.EditValue = DateTime.Today;
                lkFromStore.EditValue = null;
                lkToStore.EditValue = null;
                txtNote.Text = "";
                txtAmount.Text = "0.00";
                _details = new BindingList<MaterialTransferDetails>();
            }
        ...
        private string GenerateNextCode()
        {
            try
            {
                var all = dc.MaterialTransferList.GetBy("IsDelete = 0");
                int maxCode = 0;
                foreach (var tr in all)
                {
                    if (int.TryParse(tr.Code, out int c) && c > maxCode)
                    {
                        maxCode = c;
                    }
                }
        ...
        {
            var row = gridView1.GetRow(e.RowHandle) as MaterialTransferDetails;
            if (row == null) return;
        ...
        {
            if (lkFromStore.EditValue == null || lkToStore.EditValue == null)
            {
                XtraMessageBox.Show("يرجى اختيار مخزني المصدر والوجهة.", "بيانات ناقصة", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            if (lkFromStore.EditValue.Equals(lkToStore.EditValue))
            {
                XtraMessageBox.Show("لا يمكن التحويل لنفس المخزن.", "خطأ", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            try
            {
                ...
                _entity.TransferDate = dtTransferDate.DateTime;
                _entity.FromStoreId = lkFromStore.EditValue as int?;
                _entity.ToStoreId = lkToStore.EditValue as int?;
                ...
                if (_id > 0)
                {
                    ...
                    dc.MaterialTransferList.Edit(_id, _entity);
                }
                else
                {
                    ...
                    _id = dc.MaterialTransferList.Add(_entity);
                }

                // Delete removed items
                foreach (int detId in _deletedDetailIds)
                {
                    dc.MaterialTransferDetails.Delete(detId);
                }
                ...
                foreach (var detail in _details)
                {
                    ...
                    if (detail.Id > 0)
                    {
                        ...
                        dc.MaterialTransferDetails.Edit(detail.Id, detail);
                    }
                    else
                    {
                        ...
                        dc.MaterialTransferDetails.Add(detail);
                    }
                }
