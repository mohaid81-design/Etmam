...
namespace Etmam
{
    public partial class ucDrawingsLog : BaseUserControl
    {
        ...
        public ucDrawingsLog()
        {
            InitializeComponent();
            if (DesignMode) return;

            _canManage = PermissionService.HasPermission(PermNames.DrawingsApprovalRequest);
        ...
        private void OpenForm(int id = 0)
        {
            ...
            var handle = ShowOverlay();
            frmDrawingsAddEdit frm;
            try { frm = new frmDrawingsAddEdit(id); }
        ...
        private void bbiEdit_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            if (gridView1.GetFocusedRow() is DrawingsRegisterList rec)
                ...
                DevExpress.XtraEditors.XtraMessageBox.Show("الرجاء تحديد مخطط لتعديله.", "تنبيه",
                    MessageBoxButtons.OK, MessageBoxIcon.Warning);
        }
        ...
        private void bbiDelete_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            if (gridView1.GetFocusedRow() is not DrawingsRegisterList rec)
            {
                DevExpress.XtraEditors.XtraMessageBox.Show("الرجاء تحديد مخطط لحذفه.", "تنبيه",
                    MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            if (DevExpress.XtraEditors.XtraMessageBox.Show(
                    $"هل أنت متأكد من حذف المخطط رقم {rec.Num}؟\nسيُحذف مع بنوده ومرفقاته.",
            ...
            try
            {
                DC.DeleteDrawingsRegister(rec.Id);
                LoadData();
            }
            catch (Exception ex)
            {
