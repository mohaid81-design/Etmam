...
namespace Etmam
{
    public partial class ucMaterialIssued : DevExpress.XtraEditors.XtraUserControl
    {
        private HashSet<int> _grantedStoreIds = new();
        private bool _canManage;

        public ucMaterialIssued()
        {
            InitializeComponent();
            if (DesignMode) return;

            var dcInit = Data.DataContext.Shared;
            _canManage = PermissionService.HasPermission(PermNames.MaterialIssuedVoucher);
            ...
            // عمود colPrint → طباعة إذن الصرف المُركَّز عليه مباشرة بدون فتح النموذج
            riButtonPrint.ButtonClick += (s, e) =>
            {
                if (gridView1.GetFocusedRow() is MaterialIssuedList row)
                {
                    var handle = ShowOverlay();
                    try { MaterialIssuedPrinter.Print(row.Id); }
            ...
            {
                var row = gridView1.GetFocusedRow() as MaterialIssuedList;
                if (row != null)
                {
                    OpenAddEdit(row.Id);
                }
        ...
        private void LoadLookups()
        {
            ...
            // فلتر المخزن يعرض فقط المخازن المصرَّح للمستخدم فيها بصرف مواد (UserStoreAccess.CanIssue)
        ...
        public void LoadData()
        {
            ...
            try
            {
                ...
                var data = dc.MaterialIssuedList.GetBy(filter, new { PrjId = prjId, StoreId = storeVal }).ToList();
                foreach (var r in data)
                    r.FormattedNum = MaterialIssuedPrinter.FormatIssuedNumber(r.Num, r.IssuedDate);
            ...
            {
                XtraMessageBox.Show($"خطأ أثناء تحميل أذونات الصرف:\n{ex.Message}", "خطأ", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            finally
            {
                CloseOverlay(handle);
        ...
        private void OpenAddEdit(int id)
        {
            if (!_canManage)
            {
                XtraMessageBox.Show("ليس لديك صلاحية إدارة أذونات الصرف.", "غير مصرَّح",
                    MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            var handle = ShowOverlay();
            frmMaterialIssuedAddEdit frm;
            try { frm = new frmMaterialIssuedAddEdit(id); }
            finally { CloseOverlay(handle); }
        ...
        private void bbiEdit_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            if (row == null)
            {
                XtraMessageBox.Show("يرجى تحديد إذن صرف لتعديله.", "تنبيه", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            OpenAddEdit(row.Id);
        }
        ...
        private void btnOpen_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            if (row == null)
            {
                XtraMessageBox.Show("يرجى تحديد إذن صرف لفتحه.", "تنبيه", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            try
            {
                frm = new frmMaterialIssuedAddEdit();
        ...
        private void bbiDelete_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            if (row == null)
            {
                XtraMessageBox.Show("يرجى تحديد إذن صرف لحذفه.", "تنبيه", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            if (XtraMessageBox.Show("هل أنت متأكد من رغبتك في حذف إذن الصرف هذا؟\nسيتم حذف جميع السطور المرتبطة به.", "تأكيد الحذف", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
            {
                var handle = ShowOverlay();
                try
                {
                    Data.DataContext.Shared.DeleteMaterialIssued(row.Id);
                    XtraMessageBox.Show("تم حذف إذن الصرف بنجاح.", "حذف", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    LoadData();
                }
                catch (Exception ex)
                {
