...
namespace Etmam
{
    /// <summary>Add/Edit form for internal Material Transfer vouchers ("إذن تحويل مواد"): header (source
    /// store/destination store/date/note) + a transferred-items grid. Moves stock between two of the
    /// company's own stores — no supplier/PO concept, purely an internal warehouse movement. Mirrors
    /// frmMaterialIssuedAddEdit's stock-awareness exactly (colAddItem bulk picker, colStockQty balance
    /// display, real-time + save-time over-transfer prevention), tracked against the *source* store since
    /// that's the one being depleted.</summary>
    public partial class frmMaterialTransferAddEdit : DevExpress.XtraEditors.XtraForm
    {
        ...
        private bool _readOnly = false;                               // true عند الفتح من ucMaterialTrasfare.btnOpen: عرض فقط، بلا تعديل أو إضافة
        private List<MaterialTransferList> _list = new();
        private int _currentIndex = -1;

        private BindingList<MaterialTransferDetails> _details = new();
        private List<int> _deletedDetailIds = new();
        private List<ItemsList> _itemsCache = new();
        private ucAttachmentAddEdit? _ucAttachments;

        // رصيد كل صنف في مخزن المصدر المختار حالياً (يُحدَّث عند تغيير المخزن) — للعرض في colStockQty
        // ولمنع تحويل كمية أكبر من المتاح (انظر ValidateStockAvailability).
        ...
        // رصيد المخزون الحقيقي يستثني بالفعل ما حوَّله هذا السجل نفسه سابقاً، فتُضاف هذه الكميات مجدداً
        // عند حساب "المتاح لإعادة التحويل" أثناء التعديل.
        private Dictionary<int, decimal> _originalQtyByItemId = new();

        public frmMaterialTransferAddEdit(int id = 0)
        {
            InitializeComponent();
        ...
        private void WireEvents()
        {
            ...
            lookUpEditFromStore.EditValueChanged += (s, e) => { OnFromStoreChanged(s, e); RevalidateField(lookUpEditFromStore, IsStoreFilled(lookUpEditFromStore)); };
            lookUpEditToStore.EditValueChanged += (s, e) => { OnHeaderChanged(s, e); RevalidateField(lookUpEditToStore, IsStoreFilled(lookUpEditToStore)); };
            dateEditTransDate.EditValueChanged += (s, e) => { OnHeaderChanged(s, e); RevalidateField(dateEditTransDate, dateEditTransDate.EditValue != null && dateEditTransDate.EditValue != DBNull.Value); };
            ...
            this.FormClosing += FrmMaterialTransferAddEdit_FormClosing;
        }

        private void SetupLookups()
        {
            lookUpEditFromStore.Properties.DataSource = dc.StoreList.GetBy("IsDelete = 0");
            lookUpEditFromStore.Properties.ValueMember = "Id";
            lookUpEditFromStore.Properties.DisplayMember = "Name";
            lookUpEditFromStore.Properties.NullText = "-- اختر مخزن المصدر --";

            lookUpEditToStore.Properties.ValueMember = "Id";
            lookUpEditToStore.Properties.DisplayMember = "Name";
            lookUpEditToStore.Properties.NullText = "-- اختر مخزن الوجهة --";

            // يُمنع اختيار مخزن الوجهة قبل تحديد المصدر، ويُستبعَد المخزن المصدر نفسه من خيارات الوجهة
            // (بدل السماح باختياره ثم رفضه لاحقاً عند التحقق فقط).
            RefreshToStoreLookup();
        }

        /// <summary>يعيد تعبئة قائمة مخزن الوجهة لتستبعد المخزن المصدر المختار حالياً، ويقفل الحقل بالكامل
        /// إن لم يُختَر مصدر بعد. إن كان مخزن الوجهة المختار هو نفس المخزن المصدر الجديد، يُفرَّغ اختياره.</summary>
        private void RefreshToStoreLookup()
        {
            var fromStoreId = lookUpEditFromStore.EditValue as int?;
            lookUpEditToStore.Enabled = fromStoreId is > 0;

            var stores = dc.StoreList.GetBy("IsDelete = 0");
            lookUpEditToStore.Properties.DataSource = fromStoreId is > 0
                ? stores.Where(s => s.Id != fromStoreId.Value).ToList()
                : stores;

            if (fromStoreId is > 0 && lookUpEditToStore.EditValue is int toStoreId && toStoreId == fromStoreId.Value)
                lookUpEditToStore.EditValue = null;
        }
        ...
        private void SetupGrid()
        {
            ...
            // الديزاينر يمنع تحرير/تركيز هذا العمود (وُرِّث من قالب مشترك) — نسمح هنا بالاختيار اليدوي
            // أيضاً بجانب زر الإضافة الجماعية (colAddItem)، بنفس نهج إذن الصرف.
            colItem.OptionsColumn.AllowEdit = true;
            colItem.OptionsColumn.AllowFocus = true;
            ...
            // عمود عرض غير مرتبط بالديزاينر (بلا FieldName) — رصيد الصنف بمخزن المصدر، انظر
            // StoreBalanceHelper وRefreshStoreBalances.
            ...
            BindDetails(new List<MaterialTransferDetails>());
        }
        ...
        {
            var rec = dc.MaterialTransferList.Find(id);
            if (rec == null)
            {
            ...
            textEditNum.Text = MaterialTransferPrinter.FormatTransferNumber(rec.Num, rec.TransferDate);
            dateEditTransDate.EditValue = rec.TransferDate;
            ...
            lookUpEditFromStore.EditValueChanged -= OnFromStoreChanged;
            lookUpEditFromStore.EditValue = rec.FromStoreId;
            lookUpEditFromStore.EditValueChanged += OnFromStoreChanged;

            RefreshToStoreLookup();
            lookUpEditToStore.EditValue = rec.ToStoreId;
            memoEditPurpose.Text = rec.Note ?? "";

            LoadDetails(id);
            RefreshStoreBalances(rec.FromStoreId);
            _ucAttachments?.LoadFor("MaterialTransferList", _id);
        ...
        /// btnOpen في ucMaterialTrasfare، بديلاً عن الفتح الكامل للتعديل (bbiEdit).</summary>
        ...
        {
            lookUpEditFromStore.Properties.ReadOnly = locked;
            lookUpEditToStore.Properties.ReadOnly = locked;
            dateEditTransDate.Properties.ReadOnly = locked;
        ...
        private void LoadDetails(int id)
        {
            ...
            var list = dc.MaterialTransferDetails
                .GetBy("ParentId = @id AND IsDelete = 0", new { id })
                .OrderBy(d => d.Id)
                .ToList();

            // رصيد المخزون الحقيقي يستثني أصلاً ما حوَّله هذا السجل، فنحتفظ بكمياته الأصلية هنا كي
            // "تُضاف" مجدداً عند حساب المتاح لإعادة التحويل (انظر ValidateStockAvailability).
        ...
        private void BindDetails(List<MaterialTransferDetails> source)
        {
            _details = new BindingList<MaterialTransferDetails>(source);
        ...
        private void NewRecord()
        {
            ...
            dateEditTransDate.EditValue = DateTime.Today;

            lookUpEditFromStore.EditValueChanged -= OnFromStoreChanged;
            lookUpEditFromStore.EditValue = null;
            lookUpEditFromStore.EditValueChanged += OnFromStoreChanged;
            ...
            BindDetails(new List<MaterialTransferDetails>());
            _ucAttachments?.LoadFor("MaterialTransferList", 0);

            _currentIndex = -1;
            UpdateNavigatorCaption();
            SetDirty(false);

            lookUpEditFromStore.Focus();
        }
        ...
        private bool SaveRecord(bool silent = false)
        {
            ...
            try
            {
                ...
                Data.DataContext.RunInTransaction(tx =>
                {
                    if (isNew)
                    {
                        rec.Num = GetNextNumber(rec.TransferDate ?? DateTime.Today);
                        rec.CreatedDate = DateTime.Now;
                        rec.CreatedMachine = Session.Machine;
                        rec.CreatedBy = Session.CurrentUser?.Id ?? 1;
                        rec.IsDelete = false;
                        _id = dc.MaterialTransferList.Add(rec, tx);
                    }
                    else
                    {
                        var existing = dc.MaterialTransferList.Find(_id);
                        ...
                        dc.MaterialTransferList.Edit(_id, rec, tx);
                    }

                    SaveDetails(_id, tx);
                });

                if (isNew) textEditNum.Text = MaterialTransferPrinter.FormatTransferNumber(rec.Num, rec.TransferDate);
                _rowVersion = rec.RowVersion;

                // بعد الحفظ، أصبحت هذه الكميات هي "المحفوظة أصلاً" لهذا السجل — تُحدَّث اللقطة كي يبقى
                // حساب "المتاح لإعادة التحويل" صحيحاً إن تابع المستخدم التعديل والحفظ مجدداً بنفس الجلسة.
        ...
        {
            var helper = dc.MaterialTransferDetails;

            if (_deletedDetailIds.Count > 0) helper.DeleteRange(_deletedDetailIds, tx);
            _deletedDetailIds.Clear();

            var toAdd = new List<MaterialTransferDetails>();
            var toEdit = new List<MaterialTransferDetails>();
        ...
        {
            var row = gridView1.GetFocusedRow() as MaterialTransferDetails;
            if (row == null) return;
        ...
        /// <summary>يعبّئ الوحدة/الوصف/رصيد المخزون تلقائياً عند اختيار صنف (سواء عبر القائمة المنسدلة أو
        /// زر الإضافة بالأسفل).</summary>
        private void GridView_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            if (e.Column != colItem) return;
            if (gridView1.GetRow(e.RowHandle) is not MaterialTransferDetails row) return;
        ...
        /// الصنف بمخزن المصدر — بعد إضافة كمية الأسطر الأخرى لنفس الصنف في هذا الجدول. لا يُطبَّق قبل
        /// اختيار مخزن مصدر أو صنف للسطر (يُترك لتحقّق ValidateHeader النهائي عند الحفظ).</summary>
        private void GridView_ValidatingEditor(object sender, DevExpress.XtraEditors.Controls.BaseContainerValidateEditorEventArgs e)
        {
            ...
            if (lookUpEditFromStore.EditValue is not int fromStoreId || fromStoreId <= 0) return;
            if (gridView1.GetFocusedRow() is not MaterialTransferDetails row) return;
            ...
            {
                e.Valid = false;
                gridView1.SetColumnError(colQty, $"الكمية أكبر من المتاح بمخزن المصدر ({remaining:N2})");
            }
        }

        /// <summary>المتاح الفعلي لتحويل صنف = رصيده الحالي بمخزن المصدر + ما حوَّله هذا السجل نفسه سابقاً
        ...
        /// يضيف سطراً جديداً. مقيَّد بأرصدة مخزن المصدر — لا يظهر بالقائمة سوى الأصناف التي لها رصيد لا
        /// يساوي صفراً فيه، ولا الأصناف الموجودة أصلاً بجدول التفاصيل (تُستبعَد من القائمة نفسها عبر
        /// ExcludeItemIds بدل السماح باختيارها ثم تجاهلها).</summary>
        ...
        {
            if (lookUpEditFromStore.EditValue is not int fromStoreId || fromStoreId <= 0)
            {
                XtraMessageBox.Show("يرجى اختيار مخزن المصدر أولاً.", "تنبيه", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            using var frm = new frmItemSelect { StoreId = fromStoreId, ExcludeItemIds = existingItemIds };
            if (frm.ShowDialog(this) != DialogResult.OK) return;

            foreach (var item in frm.SelectedItems)
            {
                _details.Add(new MaterialTransferDetails
                {
                    ItemId = item.Id,
                    UnitId = item.UnitId,
        ...
        private void FetchBySearch()
        {
            ...
            var found = _list.FirstOrDefault(r =>
                ...
                MaterialTransferPrinter.FormatTransferNumber(r.Num, r.TransferDate).Contains(term, StringComparison.OrdinalIgnoreCase));
            if (found == null)
            {
        ...
        private void PrintRecord()
        {
            if (_id <= 0)
            {
                XtraMessageBox.Show("يرجى حفظ إذن التحويل قبل الطباعة.", "تنبيه", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            var handle = ShowOverlay();
            try { MaterialTransferPrinter.Print(_id); }
            finally { CloseOverlay(handle); }
        }

        // ── Required-field validation (green = required, salmon = missing on failed save) ───────

        private static bool IsStoreFilled(LookUpEdit edit) => edit.EditValue != null && edit.EditValue != DBNull.Value;
        ...
        {
            (lookUpEditFromStore as DevExpress.XtraEditors.BaseEdit, IsStoreFilled(lookUpEditFromStore)),
            (lookUpEditToStore as DevExpress.XtraEditors.BaseEdit, IsStoreFilled(lookUpEditToStore)),
            (memoEditPurpose as DevExpress.XtraEditors.BaseEdit, !string.IsNullOrWhiteSpace(memoEditPurpose.Text)),
            (dateEditTransDate as DevExpress.XtraEditors.BaseEdit, dateEditTransDate.EditValue != null && dateEditTransDate.EditValue != DBNull.Value),
        };
        ...
        private bool ValidateHeader()
        {
            ...
            if (Equals(lookUpEditFromStore.EditValue, lookUpEditToStore.EditValue))
            {
                XtraMessageBox.Show("لا يمكن أن يكون مخزن المصدر هو نفس مخزن الوجهة.", "تحقق من البيانات", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                lookUpEditToStore.Focus();
                return false;
            }
            // يُفحص فقط عند إنشاء مستند جديد. صلاحية التحويل مطلوبة من مخزن المصدر (هو من "يُخرج"
            // الرصيد)، بينما يكفي في مخزن الوجهة أن يكون المستخدم مصرَّحاً له بالاطلاع عليه فقط.
            if (_id == 0 && lookUpEditFromStore.EditValue is int fromStoreId && !InventoryStorePermissions.CanTransfer(dc, fromStoreId))
            {
                XtraMessageBox.Show("ليس لديك صلاحية تحويل من مخزن المصدر المختار.", "غير مصرَّح", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                lookUpEditFromStore.Focus();
                return false;
            }
            if (_id == 0 && lookUpEditToStore.EditValue is int toStoreId && !InventoryStorePermissions.CanView(dc, toStoreId))
            {
                XtraMessageBox.Show("ليس لديك صلاحية الاطلاع على مخزن الوجهة المختار.", "غير مصرَّح", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                lookUpEditToStore.Focus();
                return false;
            }
            if (_details.Count == 0)
            {
                XtraMessageBox.Show("يرجى إضافة صنف واحد على الأقل في جدول الأصناف المحوَّلة.", "تحقق من البيانات", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                gridControl1.Focus();
                return false;
            }
        ...
        /// <summary>يمنع حفظ تحويل كمية أكبر من المتاح فعلياً بمخزن المصدر — يُجمَع الصنف الواحد عبر كل
        /// أسطره (قد يتكرر بأكثر من سطر بمراكز تكلفة/بنود موازنة مختلفة)، ويُقارَن بالرصيد الحقيقي الحالي
        /// مضافاً إليه ما حوَّله هذا السجل نفسه سابقاً (عند التعديل)، لأن الرصيد الحقيقي يستثنيه أصلاً.</summary>
        private bool ValidateStockAvailability()
        {
            if (lookUpEditFromStore.EditValue is not int fromStoreId || fromStoreId <= 0) return true; // تحقّق منه أعلاه بالفعل

            var trueBalances = StoreBalanceHelper.ComputeBalances(dc, fromStoreId);
            ...
            foreach (var req in requestedByItem)
            {
                ...
                if (req.Qty > available)
                {
                    ...
                    XtraMessageBox.Show(
                        $"الكمية المطلوب تحويلها من الصنف [{itemName}] ({req.Qty:N2}) أكبر من المتاح بمخزن المصدر ({available:N2}).",
        ...
        // ── Entity Builder ────────────────────────────────────────────────────
        private MaterialTransferList BuildHeaderEntity()
        {
            return new MaterialTransferList
            {
                FromStoreId = lookUpEditFromStore.EditValue as int?,
                ToStoreId = lookUpEditToStore.EditValue as int?,
                TransferDate = DateTimeHelper.WithCurrentTime(dateEditTransDate.EditValue as DateTime?),
                Note = memoEditPurpose.Text?.Trim()
            };
        }

        // ── Helpers ───────────────────────────────────────────────────────────
        /// <summary>Next sequential number within the calendar year of the voucher's own TransferDate — the
        /// series resets to 1 every year (see MaterialTransferPrinter.FormatTransferNumber for the display format).</summary>
        private int GetNextNumber(DateTime transferDate)
        {
            return dc.MaterialTransferList
                .GetBy("IsDelete = 0 AND YEAR(TransferDate) = @year", new { year = transferDate.Year })
                .Select(r => r.Num ?? 0)
                .DefaultIfEmpty(0)
                .Max() + 1;
        }
        ...
        private void UpdateNavigatorCaption()
        {
            Text = _currentIndex >= 0 && _list.Count > 0
                ? $"إذن تحويل مواد رقم [{MaterialTransferPrinter.FormatTransferNumber(_list[_currentIndex].Num, _list[_currentIndex].TransferDate)}]  |  {_currentIndex + 1} / {_list.Count}"
                : "إضافة إذن تحويل مواد جديد";
        ...
        /// <summary>يعيد حساب أرصدة مخزن المصدر ويحدّث عمود colStockQty لكل سطر موجود بالفعل.</summary>
        private void OnFromStoreChanged(object? sender, EventArgs e)
        {
            OnHeaderChanged(sender, e);
            RefreshToStoreLookup();
            RefreshStoreBalances(lookUpEditFromStore.EditValue as int?);
            ClearDetailsAfterStoreChange();
        }

        /// <summary>الأرصدة المعروضة/المتحقَّق منها في الجدول كانت محسوبة على مخزن المصدر السابق — لم تعد
        /// صالحة بعد تغييره، فيُفرَّغ الجدول بدل ترك بيانات مضلِّلة. البنود المحفوظة أصلاً (Id > 0) تُضاف
        /// لقائمة الحذف كي تُزال فعلياً من قاعدة البيانات عند الحفظ، لا أن تختفي من الشاشة فقط.</summary>
        private void ClearDetailsAfterStoreChange()
        {
            ...
            XtraMessageBox.Show(
                "تم تغيير مخزن المصدر، لذا تم تفريغ جدول الأصناف تلقائياً لأن الكميات والأرصدة المعروضة كانت مرتبطة بالمخزن السابق.",
        ...
        // ── Form Closing Guard ────────────────────────────────────────────────
        private void FrmMaterialTransferAddEdit_FormClosing(object? sender, FormClosingEventArgs e)
        {
            if (_isDirty)
            {
                var result = XtraMessageBox.Show(
            ...
            // يُقرأ من ucMaterialTrasfare.OpenAddEdit لتحديث القائمة بعد أي حفظ ناجح فقط
