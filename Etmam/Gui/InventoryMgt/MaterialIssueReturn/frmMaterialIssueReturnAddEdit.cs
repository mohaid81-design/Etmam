...
namespace Etmam
{
    public class frmMaterialIssueReturnAddEdit : XtraForm
    {
        private readonly DataContext dc = Data.DataContext.Shared;
        private int _id;
        private MaterialIssueReturnList? _entity;
        ...
        private void InitializeComponent()
        {
            ...
            this.Text = $"مرتجع صرف مواد - {(_id > 0 ? "تعديل" : "جديد")}";

            // Top Panel for Header
            var pnlHeader = new GroupControl { Text = "بيانات مرتجع الصرف الرئيسية", Dock = DockStyle.Top, Height = 170, Margin = new Padding(10) };
            ...
            tableLayout.Controls.Add(new LabelControl { Text = "إلى مخزن:", Anchor = AnchorStyles.Left }, 0, 1);
            tableLayout.Controls.Add(lkStore, 1, 1);
            tableLayout.Controls.Add(new LabelControl { Text = "البيان / ملاحظات:", Anchor = AnchorStyles.Left }, 2, 1);
        ...
        private void LoadRecord()
        {
            if (_id > 0)
            {
                ...
                var list = dc.MaterialIssueReturnDetails.GetBy("ParentId = @id AND IsDelete = 0", new { id = _id });
                _details = new BindingList<MaterialIssueReturnDetails>(list);
            }
            else
            {
                _entity = new MaterialIssueReturnList();
                ...
                txtNote.Text = "";
                txtAmount.Text = "0.00";
                _details = new BindingList<MaterialIssueReturnDetails>();
            }
        ...
        private string GenerateNextCode()
        {
            try
            {
                var all = dc.MaterialIssueReturnList.GetBy("IsDelete = 0");
                int maxCode = 0;
                foreach (var ir in all)
                {
                    if (int.TryParse(ir.Code, out int c) && c > maxCode)
                    {
                        maxCode = c;
                    }
                }
        ...
        {
            var row = gridView1.GetRow(e.RowHandle) as MaterialIssueReturnDetails;
            if (row == null) return;
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                XtraMessageBox.Show("يرجى اختيار المخزن الذي سيتم إرجاع المواد إليه.", "بيانات ناقصة", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            try
            {
                ...
                if (_id > 0)
                {
                    ...
                    dc.MaterialIssueReturnList.Edit(_id, _entity);
                }
                else
                {
                    ...
                    _id = dc.MaterialIssueReturnList.Add(_entity);
                }

                // Delete removed items
                foreach (int detId in _deletedDetailIds)
                {
                    dc.MaterialIssueReturnDetails.Delete(detId);
                }
                ...
                foreach (var detail in _details)
                {
                    ...
                    if (detail.Id > 0)
                    {
                        ...
                        dc.MaterialIssueReturnDetails.Edit(detail.Id, detail);
                    }
                    else
                    {
                        ...
                        dc.MaterialIssueReturnDetails.Add(detail);
                    }
                }
